...
public class RpcTcSetClientTypeIn implements SDR {
   /** Int */
   public int clientId;
   /** RpcTcClientType */
   ...
   public void read(SDRInputStream in, SDRContext context) throws SDRException, IOException {
      ...
      actMember++;
      if (mMemberDone == actMember) {
         forceController = in.readBool(context);
         if (!context.done)
            return;
         mMemberDone++;
      }
   ...
   public void write(SDROutputStream out, SDRContext context) throws SDRException, IOException {
      ...
      actMember++;
      if (mMemberDone == actMember) {
         out.writeBool(forceController, context);
         if (!context.done)
            return;
         mMemberDone++;
      }
   ...
   public int size() {
      ...
      size += type.size();
      size += SDRUtil.sizeBool(forceController);
      return size;
   }
