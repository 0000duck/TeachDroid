...
public class RpcTcGetRoutineInfoListOut implements SDR {
   /** Int */
   public int nrOfInfos;
   public static final int cMaxLen_infos = TCI.rpcChunkLen;
   /** Array<TCI.rpcChunkLen> of RpcTcRoutineInfo */
   public RpcTcRoutineInfo[] infos;
   ...
   public void read(SDRInputStream in, SDRContext context) throws SDRException, IOException {
      ...
      if (mMemberDone == actMember) {
         ...
         if (context.done)
            infos = new RpcTcRoutineInfo[infos_count];
         if (!context.done)
            return;
         mMemberDone++;
      }
      ...
      for (int i11 = startIdx11; i11 < infos_count; i11++) {
         if (mMemberDone == actMember) {
            if (infos[i11] == null)
               infos[i11] = new RpcTcRoutineInfo();
