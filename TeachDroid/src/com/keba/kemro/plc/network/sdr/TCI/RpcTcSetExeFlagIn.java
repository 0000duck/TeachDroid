...
public class RpcTcSetExeFlagIn implements SDR {
   /** Int */
   public int clientId;
   /** Int */
   ...
   public void write(SDROutputStream out, SDRContext context) throws SDRException, IOException {
      ...
      actMember++;
      if (mMemberDone == actMember) {
         exeFlag.write(out, context);
         if (!context.done)
            return;
         mMemberDone++;
         exeFlag.reset(); //done for multiple use of input parameters
      }
      actMember++;
      if (mMemberDone == actMember) {
         out.writeBool(value, context);
         if (!context.done)
            return;
         mMemberDone++;
      }
